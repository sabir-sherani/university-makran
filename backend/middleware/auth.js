...
// Re-checks the account on every request rather than trusting the JWT
...
// Returns { user } on success, or { status, message } on failure — shared by
// makeVerifier (single fixed role) and verifyAnyRole (any of several roles),
// so the account-freshness logic exists exactly once.
async function checkAccountFresh(decoded, role) {
  const { model, activeStatuses, freshFields = [] } = ROLE_CONFIG[role];
  const user = await model().findById(decoded.id).select(['tokenVersion', 'status', 'isActive', ...freshFields].join(' '));
  if (!user) return { status: 401, message: 'Unauthorized: Account no longer exists.' };

  if ((decoded.tokenVersion || 0) !== (user.tokenVersion || 0)) {
    return { status: 401, message: 'Unauthorized: Session has been invalidated. Please log in again.' };
  }
  if (user.isActive === false) {
    return { status: 403, message: 'Forbidden: This account has been archived.' };
  }
  if (activeStatuses && !activeStatuses.includes(user.status)) {
    return { status: 403, message: 'Forbidden: Your account is not currently active.' };
  }
  return { user, freshFields };
}
...
function makeVerifier(role) {
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      const result = await checkAccountFresh(decoded, role);
      ...
      req.user = { ...decoded };
      for (const field of result.freshFields) req.user[field] = result.user[field];
...
// Accepts a request from any of the given roles (defaults to every portal
// role) instead of one fixed role — for endpoints genuinely shared across
// portals (e.g. /api/notifications) where writing five near-identical
// role-specific routers would just be makeVerifier() copy-pasted five times.
function verifyAnyRole(roles = Object.keys(ROLE_CONFIG)) {
  return async (req, res, next) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return res.status(401).json({ message: 'Unauthorized: No token provided.' });
    ...
    try {
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      if (!roles.includes(decoded.role) || !ROLE_CONFIG[decoded.role]) {
        return res.status(403).json({ message: 'Forbidden: your account type cannot access this.' });
      }

      const result = await checkAccountFresh(decoded, decoded.role);
      if (result.status) return res.status(result.status).json({ message: result.message });
